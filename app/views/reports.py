...
def index():
    ...
    sub_stats = []
    for s in subs:
        total = sum(float(p.amount or 0) for p in s.payments)
        paid = sum(float(p.amount or 0) for p in s.payments if p.status == 'paid')
        sub_stats.append({
            'sub': s,
            'total': total,
            'paid': paid,
    ...
    return render_template('reports/index.html', client_stats=client_stats, sub_stats=sub_stats)
